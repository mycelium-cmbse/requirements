...
Because Mycelium Forge owns its identity registry (§5.2.3.3) and is deployable without Mycelium Fabric, it provides the account and organization administration a standalone installation needs. These requirements are Forge's own; the equivalent Bloom and Fabric requirements (`SSS-CC-SS-HV9`, `SSS-IA-ORG-*`, `SSS-IA-USR-*`, `SSS-FB-AUTH-L0Z`) govern those components and do not satisfy a Forge-only deployment.

Registration is self-service, and an Account exists at installation level rather than inside an Organization: an Organization controls membership, not the existence of its members' Accounts. Organization creation is likewise self-service, and both Organization membership and the Organization Administrator role take effect only on the recipient's acceptance.
...
| SSS-FG-ACC-M6K | IA | Mycelium Forge shall provide operations for the Installation Administrator to deactivate, reactivate and delete any Account and to grant and revoke the Installation Administrator role, and shall prevent an operation that would remove the last Installation Administrator, when "the Installation Administrator manages Accounts." | - | H |
