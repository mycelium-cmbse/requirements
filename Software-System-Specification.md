...
##### 5.2.1.4 Collaboration and awareness
...
| SSS-PT-PUB-R7N | PA, PT, VW | Mycelium Bloom shall visually distinguish attributes with pending unpublished changes (OwnedValue differs from published value) from attributes where OwnedValue and published value are equal when "a user views attributes in the model browser, detail panel, or product tree." | - | H |  |
...
| SSS-FB-OCC-W1 | - | Mycelium Fabric shall reject a commit in which a composite (nested) feature of an occurrence is not itself an occurrence, returning a validation error identifying the offending feature, when "a client submits such a commit." | SysML 7.9 | M |  |
...
Early-phase design explores multiple solutions before committing to one. Mycelium offers two complementary mechanisms: Branches for fully independent design alternatives, and Variation Points/Variants for in-place variability within a single branch.
