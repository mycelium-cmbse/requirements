...
- **Mycelium Forge**, the package registry for publishing, discovering, and importing SysML v2 model libraries and reusable packages as well as ECSS-E-TM-10-25 and Capella models.
...
| SSS-IA-SYS-W9D | IA | Mycelium Bloom shall display installation-wide metrics including total users, total organizations, total projects, storage usage, and active sessions when "an Installation Administrator navigates to the installation dashboard." | - | M |
| SSS-IA-SYS-E3T | IA | Mycelium Bloom shall display an installation-wide audit log of administrative actions (organization creation/deletion, user creation/deactivation, role changes) when "an Installation Administrator navigates to the installation audit log view." | - | M |
...
| SSS-CC-PROF-M2F | All | Mycelium Bloom shall upload a profile avatar image, validating its format (PNG, JPG, WebP, or SVG) and maximum file size and offering a square crop, when "a user uploads or replaces their profile avatar." | - | L |
| SSS-CC-PROF-P9R | All | Mycelium Bloom shall remove the user's uploaded avatar and revert to a generated avatar derived from the user's initials when "a user removes their profile avatar." | - | L |
...
| SSS-CC-PROF-B8C | All | Mycelium Fabric shall persist a user's avatar image and expose it through a stable avatar URL referenced by the presence and co-presence indicators when "a user uploads or removes their profile avatar." | - | L |
...
| SSS-PA-PERSIST-D3J | PA, PT | Mycelium Bloom shall persist all accumulated local edits to Mycelium Fabric as a single atomic Commit on the active branch when "a user submits the batch with a commit description in batch mode." | API 7.2.3 | M |
| SSS-PA-PERSIST-C6M | PA, PT | Mycelium Bloom shall provide an input for the user to enter a commit message, stored as the description of the resulting Commit, when "a user commits one or more model changes." | API 7.2.3 | M |
| SSS-PA-PERSIST-H7T | PA, PT | Mycelium Bloom shall provide a toggle to switch between immediate mode and batch mode when "a user changes the persistence mode in the application settings or toolbar." | - | M |
| SSS-PA-PERSIST-M2F | PA, PT | Mycelium Bloom shall display a pending changes indicator showing the number of uncommitted local edits when "the user is in batch mode and has accumulated local changes." | - | M |
| SSS-PA-PERSIST-R5V | PA, PT | Mycelium Bloom shall display a list of all pending local changes with element name, change type (created, updated, deleted), and changed properties when "the user reviews the pending changes before committing a batch." | - | M |
| SSS-PA-PERSIST-N9B | PA, PT | Mycelium Bloom shall discard all pending local changes and revert to the last committed model state when "the user cancels a batch in batch mode." | - | M |
| SSS-PA-PERSIST-T1G | PA, PT | Mycelium Bloom shall warn the user about unsaved local changes when "the user attempts to close the application, switch projects, or switch branches while in batch mode with pending changes." | - | M |
...
| SSS-PT-SUB-G8P | PT | Mycelium Bloom shall delete all of the Participant's ParameterSubscriptions to attributes owned by one or more selected Ownerships in a single operation when "the Participant selects one or more owning Ownerships and invokes batch unsubscribe." | - | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PT-PUB-A3P | PA | Mycelium Bloom shall provide a setting to enable or disable auto-publish mode for a Concurrent Design project, defaulting to disabled, when "the Project Administrator edits the project's publication settings." | - | M |
...
| SSS-PA-PKG-Q1M | PA, PT | Mycelium Bloom shall support creating Alias memberships to provide alternative names for elements within a namespace when "a user assigns an alias to an imported or local element." | KerML 7.2.5.2 | M |
...
| SSS-PA-PKG-A7Q | PA, PT | Mycelium Bloom shall set the `isImportAll` flag on an Import, causing non-public Memberships to be included in the imported set, when "a user toggles the 'include private members' option on an Import." | KerML 7.2.5.4 | M |
...
| SSS-PA-PKG-X4M | PA | Mycelium Bloom shall provide a per-project preference controlling whether `SSS-PA-PKG-X2K` is enforced on every auto-Import or whether auto-Imports are created silently, with a default value of "always confirm", when "a Project Administrator edits the project settings." | KerML 7.2.5.4 | M |
...
| SSS-PA-PKG-M3G | PA, PT | Mycelium Bloom shall edit the metadata of a Package or LibraryPackage (version, author, description, license, and tags) via the detail panel when "a user edits any of the metadata fields of a Package or LibraryPackage." | - | M |
...
| SSS-PA-PKG-F8M | PA | Mycelium Bloom shall import a LibraryPackage from Mycelium Forge into the current project, creating the corresponding NamespaceImport and fetching the referenced content, when "a user selects a LibraryPackage from Mycelium Forge and invokes the 'Import Library' action." | - | M |
...
| SSS-PA-REQ-M3N | PA, PT | Mycelium Bloom shall assign one or more actors to a Requirement Definition or Requirement Usage via Actor Membership, representing external entities necessary for the requirement to be fulfilled, when "a user adds actors to a requirement." | SysML 8.3.21.2 | M |
| SSS-PA-REQ-H6W | PA, PT | Mycelium Bloom shall assign one or more stakeholders to a Requirement Definition or Requirement Usage via Stakeholder Membership, representing entities with concerns about the requirement, when "a user adds stakeholders to a requirement." | SysML 8.3.21.12 | M |
...
| SSS-PA-REQ-RF1 | PA, PT, VW | Mycelium Bloom shall display, in the detail panel of a Requirement Definition or Requirement Usage, its subject, actors, stakeholders, and framed concerns, each navigable to the referenced element, when "a user views a requirement." | SysML 8.3.21 | M |
...
| SSS-PA-ELEM-R1D | PA, PT, VW | Mycelium Bloom shall display the full details of a selected element, including its name, short name, type, multiplicity, Ownership, documentation, attributes, and relationships, in the detail panel when "a user selects an element in any view." | - | M |
...
| SSS-PA-ELEM-RU2 | PA, PT, VW | Mycelium Bloom shall display whether a Usage is composite or referential using a distinguishing indicator, showing the `ref` keyword in the textual notation and a reference marker in the model browser, detail panel, and diagrams, when "a user views a Usage." | KerML 7.3.4.2 | M |
...
| SSS-PA-ARCH-N5W | PA, PT | Mycelium Bloom shall create a Featuring relationship establishing that one type features another type when "a user explicitly specifies a featuring relationship between two types." | KerML 7.3.4.8 | M |
...
| SSS-PA-ELEM-R4S | PA, PT | Mycelium Bloom shall create a ReferenceSubsetting on a reference Feature of a Connection end, Flow end, or Interface end, identifying the participating Feature that the end refers to, when "a user sets the referent of a connector, flow, or interface end." | KerML 8.3.3.3.9 | M |
| SSS-PA-ELEM-C5X | PA, PT | Mycelium Bloom shall create a CrossSubsetting on a cross Feature of an Association Definition when "a user designates a Feature as the cross-feature of an Association between two participating Types." | KerML 8.3.3.3.2 | M |
...
| SSS-PA-ELEM-V3W | PA, PT, VW | Mycelium Bloom shall display, in the detail panel of the owning element, a warning for any Feature whose actual value count falls outside the literal bounds of its Multiplicity Range when "a user runs model validation or opens the detail panel of such a Feature." | KerML 7.4.12 | M |
...
| SSS-PA-ELEM-P6Q | PA, PT, VW | Mycelium Bloom shall display the parameter Features of an Action, Calculation, Function, or Case, derived from their ParameterMemberships, with each parameter's direction, type, and multiplicity, in the detail panel, when "a user views an Action, Calculation, Function, or Case." | KerML 7.4.7 | M |
| SSS-PA-ELEM-R7S | PA, PT, VW | Mycelium Bloom shall display the return Feature of a Function or Calculation, derived from its ReturnParameterMembership, with its type and multiplicity, in the detail panel, when "a user views a Function or Calculation." | KerML 7.4.8 | M |
| SSS-PA-ELEM-X8T | PA, PT, VW | Mycelium Bloom shall display the result expression of a Calculation or Boolean Expression, derived from its ResultExpressionMembership, in its textual form, in the detail panel, when "a user views a Calculation or Boolean Expression." | KerML 7.4.8 | L |
...
| SSS-PA-ELEM-N3J | PA, PT, VW | Mycelium Bloom shall reveal and select an element on every visible diagram that contains it when "a user invokes show-on-diagram on an element selected in the browser or a tabular view." | - | M |
...
| SSS-PA-VAR-H2J | PA, PT, VW | Mycelium Bloom shall visually distinguish variation points from regular Usages in the model browser, tabular view and on diagrams using a distinct indicator when "the model contains Usages with `isVariation = true`." | SysML 7.6 | M |
| SSS-PA-VAR-M8F | PA, PT, VW | Mycelium Bloom shall display all variant Usages nested under a variation point as selectable alternatives when "a user expands a variation point in the model browser." | SysML 7.6 | M |
| SSS-PA-VAR-D4B | PA, PT | Mycelium Bloom shall select an active variant for a variation point, filtering the model browser, tabular view, diagrams, and calculations to show only the selected variant's content, when "a user selects a variant from the variant selector of a variation point." | SysML 7.6 | M |
...
| SSS-PA-VAR-J9K | PA, PT | Mycelium Bloom shall propagate structural changes (added attributes, ports, nested usages) from the variation point to all its variants when "a user modifies the shared structure of a variation point." | SysML 7.6 | M |
| SSS-PA-VAR-F1P | PA, PT | Mycelium Bloom shall override attribute values on individual variant Usages without affecting other variants or the variation point definition when "a user edits an attribute value on a specific variant." | SysML 7.6 | M |
| SSS-PA-VAR-B3G | PA, PT, VW | Mycelium Bloom shall indicate in the Browser View which variant is active for each variation point using visual markers when "the model contains variation points with a selected active variant." | SysML 7.6 | M |
...
| SSS-PA-VAR-C5H | PA, PT, VW | Mycelium Bloom shall display a resolved decomposition tree showing the full system decomposition with only the selected variants included, as if the configuration were the actual design, when "a user applies a variant configuration." | SysML 7.6 | M |
...
| SSS-PA-VAR-CC2 | PA, PT | Mycelium Bloom shall create constraints that restrict which variants may be selected together across variation points, for example a six-cylinder engine requiring an automatic transmission, when "a user adds a configuration constraint between variants." | SysML 7.6 | M |
| SSS-PA-VAR-CV3 | PA, PT, VW | Mycelium Bloom shall evaluate whether a selected configuration satisfies all configuration constraints and indicate any invalid or conflicting selections when "a user selects variants to define a configuration." | SysML 7.6 | M |
| SSS-PA-VAR-RR4 | PA, PT | Mycelium Bloom shall compute the resolved model for a selected configuration by including only the selected variant at each variation point, applying that variant's redefinitions, recursively resolving nested variations, and resolving variants declared by reference, when "a user applies a variant configuration." | SysML 7.6 | M |
...
| SSS-PA-TRACE-W9G | PA, PT, VW | Mycelium Bloom shall sort the Relationship Matrix rows and columns by element name, namespace path, or relationship count when "a user changes the sort order of the Relationship Matrix." | - | M |
...
| SSS-CC-EXT-5DV | PA, PT | Mycelium Bloom shall support External Relationships linking model elements to external web resources via IRIs when "a user creates a relationship targeting an external resource identified by an IRI." | API 7.2.5 | M |
| SSS-PA-TRACE-RX1 | PA, PT, VW | Mycelium Bloom shall display, in the detail panel of an element, the typed relationships, dependencies, and featurings in which it participates, each navigable to the related element, when "a user views an element that participates in relationships." | KerML 7.2.3 | M |
...
| SSS-PA-VIS-F8Q | PA, PT | Mycelium Bloom shall provide operations to place free-text notes on the diagram canvas, with optional formatting (bold, italic, color), when "a user creates a note on a diagram." | KerML 7.2.4 | M |
| SSS-PA-VIS-B2M | PA, PT | Mycelium Bloom shall attach a note to a specific model element on the diagram via a dashed anchor line when "a user links a note to a diagram element." | KerML 7.2.4 | M |
| SSS-PA-VIS-G5R | PA, PT | Mycelium Bloom shall provide callout annotations that can point to a specific location on the diagram canvas when "a user creates a callout on a diagram." | KerML 7.2.4 | M |
| SSS-PA-VIS-T1J | PA, PT | Mycelium Bloom shall persist diagram notes and callouts as SysML v2 Comment elements annotating the relevant model elements when "a user saves a diagram containing notes or callouts." | KerML 7.2.4 | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-VIS-UC1 | PA, PT | Mycelium Bloom shall provide an editor for creating and editing Use Case Views showing use cases, actors, the subject boundary, and their relationships when "a user opens or creates a Use Case View for a selected subject." | SysML 8.2.3.25 | M |
| SSS-PA-VIS-UC2 | PA, PT, VW | Mycelium Bloom shall render Use Case Usages as ovals enclosed within the subject boundary rectangle, Actors as stick figures connected by association lines, and «include» relationships as dashed arrows, using the SysML v2 graphical notation (section 8.2.3.25), when "a Use Case View displays use case model content." | SysML 8.2.3.25 | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-VIS-JPW | PA, PT | Mycelium Bloom shall provide a Grid View for tabular and matrix representations of model data when "a user creates a Grid View and selects the element types and properties to display." | SysML 7.26 | M |
...
| SSS-PA-VIS-T1N | PA, PT, VW | Mycelium Bloom shall export a diagram to PNG format at a user-specified resolution when "a user selects PNG as the export format for a diagram." | - | M |
| SSS-PA-VIS-G4L | PA, PT, VW | Mycelium Bloom shall export a diagram to JPG format at a user-specified resolution and quality when "a user selects JPG as the export format for a diagram." | - | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-VIS-E8Q | PA, PT, VW | Mycelium Bloom shall provide an interactive 3D viewer that renders the system decomposition as a three-dimensional scene when "a user opens the 3D model viewer for a project." | - | M |
| SSS-PA-VIS-M2K | PA, PT, VW | Mycelium Bloom shall render Part Usages in the 3D viewer using the geometric Attribute Usages defined by `SSS-PA-VIS-G1A` and `SSS-PA-VIS-G2B` as the primary source, falling back to placeholder shapes per `SSS-PA-VIS-G6F` when the required Attribute Usages are absent, when "the 3D viewer displays model elements." | - | M |
| SSS-PA-VIS-R5N | PA, PT, VW | Mycelium Bloom shall provide camera controls for orbiting, panning, and zooming the 3D scene when "a user interacts with the 3D viewer using mouse or touch input." | - | M |
| SSS-PA-VIS-W1J | PA, PT, VW | Mycelium Bloom shall highlight the selected element in the 3D scene and display its properties in the detail panel when "a user selects a model element in the 3D viewer." | - | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-QRY-L11 | PA, PT | Mycelium Bloom shall provide a query interface supporting select, scope, where, and orderBy clauses when "a user composes a query and submits it for execution." | API 7.2.4 | L |
| SSS-PA-QRY-JYA | PA, PT, VW | Mycelium Bloom shall execute queries against any commit to retrieve historical model state when "a user specifies a target Commit identifier before executing a query." | API 7.2.4 | L |
| SSS-PA-QRY-QR1 | PA, PT, VW | Mycelium Bloom shall display query results as a sortable, filterable table from which the user can navigate to any matching element when "a query completes execution." | API 7.2.4 | L |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-OA-SYS-IEJ | OA | Mycelium Bloom shall display active user sessions and system health metrics when "the Organization Administrator navigates to the system monitoring dashboard." | - | M |
...
| SSS-PA-DASH-R9V | PA, PT, VW | Mycelium Bloom shall display a pie chart showing the distribution of model elements by element type (Part, Item, Action, State, Requirement, Constraint, etc.) when "the project model dashboard renders the element composition section." | - | M |
| SSS-PA-DASH-T4K | PA, PT, VW | Mycelium Bloom shall display a pie chart showing the distribution of model elements by Ownership when "the project model dashboard renders the ownership distribution section." | - | M |
...
| SSS-PA-DASH-B7N | PA, PT, VW | Mycelium Bloom shall navigate to the list of matching model elements when "a user clicks a bar in a histogram or a segment in a pie chart on the project model dashboard." | - | M |
...
| SSS-PA-HIST-T6W | PA, PT, VW | Mycelium Bloom shall render multiple attributes of different quantity kinds on the same chart using independent Y-axes (one per quantity kind) with distinct colours and a shared Commit/Tag X-axis when "a user selects attributes with different units or quantity kinds for the attribute history view." | - | L |
| SSS-PA-HIST-V2P | PA, PT, VW | Mycelium Bloom shall display the unit and quantity kind label on each Y-axis and provide a legend identifying each plotted attribute by name, element, and unit when "the attribute history chart displays multiple attributes." | - | L |
...
| SSS-PA-REQ-D7W | PA | Mycelium Bloom shall export requirements to ReqIF format when "the Project Administrator initiates an export operation and selects a target ReqIF file or target location." | - | M |
...
| SSS-PA-CMT-K2B | PA, PT | Mycelium Bloom shall format Comment and Documentation text using rich text (bold, italic, lists, links) when "a user edits the body of a Comment or Documentation element." | KerML 7.2.4 | M |
| SSS-PA-CMT-D5P | PA, PT | Mycelium Bloom shall annotate a single Comment on multiple model elements simultaneously when "a user creates a comment and selects multiple annotated elements." | KerML 7.2.4.2 | M |
| SSS-PA-CMT-N8V | PA, PT | Mycelium Bloom shall specify an optional locale (e.g. "en", "fr", "de") on a Comment or Documentation element when "a user sets the language of a comment or documentation entry." | KerML 7.2.4.2 | L |
| SSS-PA-CMT-Y6L | PA, PT | Mycelium Bloom shall create a Textual Representation on a model element, embedding language-specific text (e.g. a code snippet, formula, or DSL expression) tagged with the language identifier, when "a user adds a textual representation to an element and selects the language." | KerML 7.2.4.3 | M |
...
| SSS-PA-CMT-RP1 | PA, PT | Mycelium Bloom shall create a reply to an existing Comment, modelled as a Comment annotating the parent Comment, forming a threaded discussion, when "a user replies to a Comment." | KerML 7.2.4 | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-ATT-W5R | PA, PT | Mycelium Bloom shall upload one or more file attachments to any model element when "a user selects a model element and adds attachments via the attachment interface." | - | M |
| SSS-PA-ATT-K3J | PA, PT, VW | Mycelium Bloom shall display a list of all attachments associated with a model element, showing file name, file type, size, upload date, and uploading user, when "a user views the attachments of a model element." | - | M |
| SSS-PA-ATT-M8D | PA, PT, VW | Mycelium Bloom shall download an attachment when "a user selects an attachment from the attachment list of a model element." | - | M |
| SSS-PA-ATT-F2N | PA, PT | Mycelium Bloom shall remove an attachment from a model element when "a user with write access to the element deletes an attachment from the attachment list." | - | M |
...
| SSS-PA-ATT-VR1 | PA, PT | Mycelium Bloom shall upload a new revision of an existing attachment, preserving prior revisions, when "a user replaces an attachment with an updated file." | - | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-GLOSS-T5R | PA, PT | Mycelium Bloom shall provide operations to create and manage a Glossary Package containing Item Definitions where each Item Definition represents a glossary term (name = term, short name = abbreviation) with Documentation providing the definition when "a user accesses the glossary management interface." | KerML 7.2.4 | L |
| SSS-PA-GLOSS-K2W | PA, PT, VW | Mycelium Bloom shall display a tabular view listing all glossary terms with their name, abbreviation, definition, and owning package when "a user opens the glossary browser." | KerML 7.2.4 | L |
| SSS-PA-GLOSS-H8N | PA, PT | Mycelium Bloom shall provide operations to create, edit, and delete glossary terms (Item Definitions with Documentation) within a Glossary Package when "a user accesses the glossary management interface." | KerML 7.2.4 | L |
| SSS-PA-GLOSS-M3J | PA, PT, VW | Mycelium Bloom shall render any occurrence of a glossary term name or abbreviation as highlighted linked text throughout the application (model browser, detail panels, requirement text, diagram labels, comments) when "text content contains a word matching a glossary term name or abbreviation." | KerML 7.2.4 | L |
| SSS-PA-GLOSS-V9D | PA, PT, VW | Mycelium Bloom shall display a tooltip showing the glossary term definition when "a user hovers over a highlighted glossary term in any view." | KerML 7.2.4 | L |
| SSS-PA-GLOSS-F6B | PA, PT, VW | Mycelium Bloom shall navigate to the glossary term's Item Definition in the glossary browser when "a user clicks a highlighted glossary term link." | KerML 7.2.4 | L |
...
| SSS-PA-VC-KXT | PA | Mycelium Bloom shall provide a configuration interface for branch protection rules, including the required number of approving reviews, on any branch when "the Project Administrator accesses the branch protection settings." | - | M |
...
| SSS-VW-VH-WGA | PA, PT, VW | Mycelium Bloom shall display the commit history of a project when "a user navigates to the version history view." | API 7.2.3 | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-CC-BACK-R5W | All | Mycelium Bloom shall connect to any backend server that implements the OMG Systems Modelling API and Services specification (REST/HTTP PSM), not only Mycelium Fabric, when "a user provides the URL of a compliant model server." | API 7 | M |
| SSS-CC-BACK-K8N | All | Mycelium Bloom shall detect whether the connected backend supports push-based change notifications (e.g. SignalR/WebSocket) and fall back to a polling mechanism when "the backend does not offer push-based notifications." | - | M |
| SSS-CC-BACK-D3T | All | Mycelium Bloom shall poll the connected backend for model changes at a user-configurable interval when "the polling mechanism is active." | - | M |
| SSS-CC-BACK-H7J | All | Mycelium Bloom shall provide a setting to configure the polling interval (in seconds) and to enable or disable polling when "a user accesses the connection settings for a backend." | - | M |
...
| SSS-CC-BACK-AU1 | All | Mycelium Bloom shall authenticate to a connected backend using the credentials or token configured for that backend when "a user connects to a backend that requires authentication." | - | M |
...
| SSS-OA-AUTH-GIT | All | Mycelium Fabric shall enforce password policies and session expiration rules when "the Organization Administrator has configured security policies in the authentication settings." | - | M |
...
| SSS-FB-ATT-T4X | All | Mycelium Fabric shall enforce ownership-based access control on attachment operations (upload, delete) consistent with the element's Owner metadata when "a user attempts to modify attachments on a model element." | - | M |
...
The package registry shall also support and host other MBSE artefacts or models such as ECSS-E-TM-10-25 models produced by CDP4-COMET and Capella models. Effectively, this makes Forge more than a package manager but a "Model Hub". Here, users can share not only kerml and sysml2 content, but also other types of MBSE models.

**Finding a package and resolving an element are two capabilities, not one.** Free-text search (`SSS-FG-REG-Q7G`, `SSS-FG-REG-W9J`) is ranked by relevance and matches package metadata only; qualified-name resolution (`SSS-FG-REG-Z5Q`) is an exact or prefix lookup over element names and is not ranked at all. They are separated because element names are a poor signal for *package* relevance and the error is not random: it favours the largest and most general libraries, so a quantities library would match nearly any physics-adjacent query while being the least useful answer. Resolution answers a different question — which package version defines this element — and that question has a right answer rather than a ranking.

| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-FG-REG-K1A | - | Mycelium Forge shall accept and distribute every published SysML v2 library as a single **kpar** file (KerML Project Archive, defined in KerML clause 10.3) whose structure, manifest, and metadata conform to the kpar specification, when "a client publishes or retrieves a library." | KerML 7.4.14, 8.3.4.13.3 | M |
| SSS-FG-REG-E5T | - | Mycelium Forge shall accept and distribute every published ECSS-E-TM-10-25 model as a single Annex C.3 exchange file (defined in ECSS-E-TM-10-25, Annex C.3) whose archive structure and root Header.json conform to the Annex C.3 specification, and shall record its package name, version and licence as publisher-supplied metadata, frozen with the version, when "a client publishes or retrieves a model". | - | M |
| SSS-FG-REG-C7P | - | Mycelium Forge shall accept and distribute every published Capella model as a single Capella project archive whose structure conforms to the Capella model and library layout,  and shall record its package name, version and licence as publisher-supplied metadata, pre-filled from the model where the reader offers a value, frozen with the version, when  "a client publishes or retrieves a model". | - | M | 
| SSS-FG-REG-S2B | - | Mycelium Forge shall assign every package version a SemVer 2.0.0 string of the form `MAJOR.MINOR.PATCH[-prerelease][+build]` and shall reject any publish request whose version string is not SemVer-compliant, whose version is not strictly greater than every previously published version of the same package, or whose major version change is not accompanied by release notes describing the breaking change, when "an authenticated client submits a publish request." | - | M |
| SSS-FG-REG-I3C | - | Mycelium Forge shall treat every accepted `{package identifier, version}` pair as immutable: once published, its kpar content and its manifest shall not be mutated. A correction to a published version requires publishing a new version, when "a client attempts to republish an existing `{package identifier, version}`." | - | M |
...
| SSS-FG-REG-A5E | - | Mycelium Forge shall expose an HTTP API endpoint that accepts a kpar file and publisher credentials and atomically registers the kpar either as the first version of a new package or as a new version of an existing package, returning the assigned download URL and the canonical manifest, when "an authenticated client pushes a kpar to the publish endpoint." | - | M |
| SSS-FG-REG-D6F | - | Mycelium Forge shall expose HTTP API endpoints returning the kpar content of a package by `{package identifier, version}` and, given a package identifier alone, the kpar content of the latest non-prerelease, non-unlisted version of that package, when "a client requests a package from the download endpoint." | - | M |
| SSS-FG-REG-Q7G | - | Mycelium Forge shall expose an HTTP search API accepting free-text query terms matched against package identifier, display name, description, tags, and authors, with paginated results, configurable sort (relevance, downloads, last-updated, name), and optional filters on license, author, tag, and updated-since timestamp, when "a client queries the search endpoint." | - | M |
| SSS-FG-REG-Z5Q | - | Mycelium Forge shall expose an HTTP endpoint that resolves a qualified element name to every package version defining it, by exact or by prefix match, returning all matches unranked and without selecting one on the caller's behalf, with paginated results, when "a client resolves a qualified element name." | - | M |
| SSS-FG-REG-M8H | - | Mycelium Forge shall expose HTTP API endpoints returning the full manifest, the complete version list, the dependency graph, and the per-version release notes of a package without requiring the kpar content itself to be downloaded, when "a client queries package metadata." | - | M |
| SSS-FG-REG-W9J | IA, OA, PA, PT, VW | Mycelium Forge shall provide a public web-based user interface, reachable by unauthenticated and authenticated users, that supports searching for packages by free-text query matched against package identifier, display name, description, and tags, with paginated and sortable results, when "a user visits the Forge web interface." | - | M |
| SSS-FG-REG-X1K | IA, OA, PA, PT, VW | Mycelium Forge's web interface shall present, for each package, a detail page displaying the package identifier, display name, description, README, authors, license, tags, the full version history with per-version release notes and publication dates, dependency declarations, download counts, the latest stable version, and a direct download link for each listed version, when "a user opens a package in the Forge web interface." | - | M |
| SSS-FG-REG-Y2L | IA, OA, PA | Mycelium Forge shall authenticate publishing clients using revocable API keys issued from the user's Mycelium account through the Forge web interface, scoping each key to a publisher and to a permitted set of operations (publish, unlist, manage-keys), when "a client submits a publish, unlist, or key-management request." Read access to public packages through the web interface, HTTP API, and download endpoint shall be permitted without authentication. | - | M |
| SSS-FG-REG-C3M | - | Mycelium Forge shall provide a first-party client library for the .NET ecosystem,  distributed as a NuGet package, covering search, package metadata, version listing, artefact download, publish, unlist, and API key issuance and revocation, and configured from a base URL alone, when "a client integrates with the registry programmatically from .NET." | - | M |
| SSS-FG-REG-C5L | - | Mycelium Forge shall provide a first-party command-line client covering the operations of SSS-FG-REG-C3M together with local credential storage, distributed both via NuGet as a .NET tool and as self-contained binaries requiring no .NET SDK, offering the same commands at the same version on both channels, when "a user or a CI/CD pipeline operates on the registry from a shell." | - | M |
| SSS-FG-REG-J4V | - | Mycelium Forge shall provide a first-party client library for the Java ecosystem, distributed as a Maven artefact and covering the operations of SSS-FG-REG-C3M, together with the integration by which the open-source Java reference implementation of SysML v2 resolves and retrieves packages from a Forge registry, when "a client integrates with the registry  programmatically from Java." | - | L |
| SSS-FG-REG-T8S | - | Mycelium Forge shall provide a first-party client library for the TypeScript ecosystem, distributed as an npm package and covering the operations of SSS-FG-REG-C3M, together with a Visual Studio Code extension through which a user searches, retrieves and publishes Forge packages without leaving the editor, when "a client integrates with the registry programmatically from TypeScript, or works with Forge packages in Visual Studio Code." | - | L |
| SSS-FG-REG-N2C | - | Mycelium Forge shall serve artefacts belonging to packages that are not public with cache directives that prevent storage by shared caches, when "a client downloads an artefact." | - | M |
| SSS-FG-REG-H8F | - | Mycelium Forge shall restrict content-hash substitution to artefacts the requesting principal is authorised to read, and shall fail resolution where no authorised byte-identical copy exists, when "a requested package version cannot be served." | - | M |
| SSS-FG-REG-M5R | IA | Mycelium Forge shall replicate from an upstream registry only those packages that its configured upstream credential is authorised to read, when "an installation configured as a mirror replicates the upstream metadata index." | - | M |
| SSS-PA-REG-B4N | PA, PT | Mycelium Bloom shall provide a Forge package picker that issues searches against Mycelium Forge via the first-party Forge client library, presents matching packages with their description, latest stable version, authors, license, and tags, and, upon confirmation, imports the selected package(s) into the current project through the auto-import flow defined by `SSS-PA-PKG-F8M`, `SSS-PA-PKG-X1J`, and `SSS-PA-PKG-X2K`, when "a user opens the 'Import from Forge' dialog." | KerML 7.2.5 | M |
| SSS-PA-REG-V5P | PA, PT, VW | Mycelium Bloom shall display, per project, the list of Forge packages currently imported, showing each package's pinned version, latest available stable version, publication date, authors, license, and an explicit 'Update' action when a newer compatible stable version is available, when "a user views the project's Forge dependencies." | KerML 7.4.14 | M |
...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-PA-IE-OYJ | PA | Mycelium Forge shall provide standard SysML v2 libraries (e.g. Quantities and Units, standard view definitions) for import into a project when "the Project Administrator selects one or more standard libraries for import." | SysML 9.8 | M |
| SSS-FG-PKG-P7L | - | Mycelium Forge shall publish a LibraryPackage as a versioned, downloadable package (including its metadata, content, and transitive library dependencies) when "a user or CI pipeline submits a LibraryPackage for publication to Mycelium Forge." | - | M |

##### 5.2.3.3 Authentication and authorization

Mycelium Forge owns its identity registry. It ships its own identity provider and holds its own Accounts and Organizations, so that a Forge installation is deployable without Mycelium Fabric; where an upstream identity provider is present, Forge federates to it as configuration rather than depending on it, in the sense `SSS-CC-EXT-ID1` gives to Keycloak's upstream identity federation. On top of that, Forge layers a per-package role model: a package has a set of Maintainers drawn from Accounts and Organizations, at least one individual-Account Owner must always exist, and ownership is transferable between Accounts and Organizations with explicit acceptance by the receiving party.

Package **visibility** governs who may read a package; a **role** governs what a principal may do with one. The two are independent, and both are independent of unlisting, which is a deprecation signal rather than an access control.

| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-FG-AUTH-S1A | IA, OA, PA, PT, VW | Mycelium Forge shall authenticate users over OIDC against the identity provider it ships with, and shall federate to an upstream identity provider where one is configured, so that a Forge installation operates without requiring Mycelium Fabric to be present, when "a user signs in to Mycelium Forge through the web interface, the HTTP API, or the Forge client library." | - | H |
| SSS-FG-AUTH-A2F | IA | Mycelium Forge shall provision a Forge Account on a user's first successful sign-in, and shall establish the initial Installation Administrator from deployment configuration rather than from the first user to complete setup, when "a user signs in for the first time, or an installation is initialised." | - | H |
| SSS-FG-AUTH-S2B | - | Mycelium Forge shall use a scoped package identifier of the form `@<scope>/<package-name>`, where `<scope>` resolves to the slug of a Forge Account or a Forge Organization, reserving the unscoped (global) namespace for standard libraries distributed by the Mycelium operator, when "a client publishes, queries, or downloads a package." | - | H |
| SSS-FG-AUTH-M3C | - | Mycelium Forge shall maintain, for every package, a Maintainer set whose entries are Forge Accounts and/or Organizations, each entry holding the role `Owner` or `Maintainer`. An `Owner` may transfer or share ownership, add or remove Maintainers, change the package's visibility, and unlist versions. A `Maintainer` may publish new versions and unlist versions but shall not modify the Maintainer set or the package's visibility. A package's metadata, display name, description, authors, license, tags, dependencies, and release notes, shall be sourced from the artefact's manifest where the format carries one and supplied by the publisher at publish time where it does not, shall record which of the two applies, and shall not be edited by any role outside of publishing a new version, when "any authenticated request operates on a package's content or ownership." | - | H |
| SSS-FG-AUTH-O4D | - | Mycelium Forge shall reject any operation, removal of a Maintainer, role downgrade, ownership transfer, or Account deletion, that would leave a package with zero individual-Account Owners, and shall require the operation to first install another individual-Account Owner, when "an authenticated client submits a change to a package's Maintainer set or Mycelium Forge deletes an Account that is the last individual Owner of one or more packages." | - | H |
| SSS-FG-AUTH-T5E | - | Mycelium Forge shall transfer or share Ownership of a package only after the receiving Account or the receiving Organization has explicitly accepted the transfer through the Forge web interface or the Forge client library, leaving the original Maintainer set unchanged until acceptance occurs, when "an Owner initiates a transfer of, or an addition to, a package's Owner set." | - | H |
| SSS-FG-AUTH-G6F | OA | Mycelium Forge shall accept an Organization as a Maintainer or Owner of a package, granting publish, unlist, and, when the role is `Owner`, ownership-management authority to the Organization's Organization Administrators and to any Organization Member explicitly granted the `Forge Publisher` role by an Organization Administrator, when "an authenticated member of an Organization that holds such a role submits an operation against the package." | - | H |
| SSS-FG-AUTH-P7G | - | Mycelium Forge shall treat an Organization entry in a package's Maintainer set as a group Owner that does not satisfy the 'at least one individual Owner' invariant of `SSS-FG-AUTH-O4D` on its own; an individual-Account Owner shall remain present alongside any Organization Owner, when "a Maintainer set is established or modified." | - | H |
...
| SSS-FG-AUTH-V6K | IA, OA | Mycelium Forge shall support three package visibility levels, private, organization-visible, and public, shall create every new package as private unless the owning Organization has configured a different default, and shall provide operations for a package Owner to change a package's visibility, when "a package is created or its settings are changed." | - | H |
| SSS-FG-AUTH-D4M | All | Mycelium Forge shall exclude packages that the requesting principal is not authorised to read from search results, from qualified-name resolution, and from metadata responses, and shall return a response indistinguishable from that returned for a package that does not exist, when "a principal requests a package it is not authorised to read." | - | H |
| SSS-FG-AUTH-K9R | IA, OA | Mycelium Forge shall extend the Maintainer set of `SSS-FG-AUTH-M3C` with the role `Reader`, which grants read access to a package that the principal's visibility would otherwise exclude and confers no write authority, and shall reject the assignment of `Reader` on a public package, when "an Owner modifies a package's Maintainer set." | - | H |
| SSS-FG-AUTH-E7N | IA | Mycelium Forge shall prevent any principal other than the Installation Administrator from deleting a package or erasing a published version, shall offer unlisting as the only withdrawal available to a package's Owners and Maintainers, and shall record every deletion and erasure in the audit log, when "a principal requests the deletion of a package or a published version." | - | H |

##### 5.2.3.4 Account and organization management

Mycelium Forge owns its identity registry (§5.2.3.3), and a Forge installation can be deployed on its own, with neither Mycelium Bloom nor Mycelium Fabric present. It therefore needs its own account and organization administration.

**The capabilities below are deliberately specified a second time, and this is not a duplication error.** The SSS already requires account and organization administration of Bloom (`SSS-CC-SS-HV9` creates Organizations, `SSS-IA-ORG-*` and `SSS-IA-USR-*` administer them) and of Fabric (`SSS-FB-AUTH-L0Z` sends organization invitations). Each of those requirements is discharged by the component it names. A Forge-only installation contains none of those components, so none of those requirements is discharged there — and without the requirements in this subsection such an installation would hold Organizations it had no means to create and expect Accounts it had no means to register. The repetition is *across components*, each carrying its own obligation, not a restatement of one obligation.

Three principles run through the requirements below:

- **Registration and Organization creation are self-service.** Neither requires an administrator. The Installation Administrator can create an Account, but ordinary users do not depend on that route.
- **Membership is accepted, never imposed.** An invitation to an Organization, and a grant of the Organization Administrator role, take effect only when the recipient accepts.
- **An Account exists at installation level, not inside an Organization.** An Organization controls who belongs to it and with what role; it does not control whether its members' Accounts exist. Removing a member does not remove their Account.

| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-FG-ACC-R1B | All | Mycelium Forge shall create an Account from a registration comprising a valid email address and an installation-unique username, shall send a verification message to that address, and shall withhold publishing and package-role assignment from the Account until the address is verified, when "a visitor submits a registration request." | - | H |
| SSS-FG-ACC-P2D | All | Mycelium Forge shall provide operations for an Account holder to create, update and clear their own profile, comprising biography, company, location, website, social media links and avatar, and shall display that profile on the Account's page, when "an Account holder manages their profile, or a visitor views an Account." | - | M |
| SSS-FG-ACC-C3F | All | Mycelium Forge shall provide operations for an Account holder to change their username and email address, to deactivate their Account, and to delete their Account, and shall refuse a deactivation or deletion that would leave a package without an individual-Account Owner as required by `SSS-FG-AUTH-O4D`, when "an Account holder manages their own Account." | - | H |
| SSS-FG-ACC-V4H | All | Mycelium Forge shall display to an Account holder every Organization the Account belongs to together with the role held in each, when "an Account holder views their memberships." | - | M |
| SSS-FG-ACC-L5J | IA | Mycelium Forge shall display to the Installation Administrator a list of all Accounts on the installation showing username, email address, verification status, Organization memberships with role, and Account status, with filtering and pagination, when "the Installation Administrator opens the account administration interface." | - | H |
| SSS-FG-ACC-M6K | IA | Mycelium Forge shall provide operations for the Installation Administrator to create, deactivate, reactivate and delete any Account, to record an Account's email address as verified without a verification exchange, and to grant and revoke the Installation Administrator role, and shall prevent an operation that would remove the last Installation Administrator, when "the Installation Administrator manages Accounts." | - | H |
| SSS-FG-ORG-C1M | All | Mycelium Forge shall create an Organization from a requested slug and display name, shall assign the requesting Account as its Organization Administrator, and shall reserve the slug as that Organization's package scope, when "an authenticated Account submits an Organization creation request." | - | H |
| SSS-FG-ORG-S2P | IA, OA | Mycelium Forge shall reject an Organization slug that is already in use by an Account, by another Organization, or by a scope proxied from an upstream registry, when "an Account requests an Organization slug or the Installation Administrator reserves one." | - | H |
| SSS-FG-ORG-U3R | OA | Mycelium Forge shall provide operations for an Organization Administrator to update an Organization's display name, description and profile and to delete the Organization, and shall refuse deletion while the Organization's scope holds packages, when "an Organization Administrator manages the Organization." | - | H |
| SSS-FG-ORG-I4T | OA | Mycelium Forge shall send an invitation to an Account to join an Organization with a specified role, shall admit the Account to the Organization only upon the invitee's explicit acceptance, and shall expire an unaccepted invitation after a configurable period, when "an Organization Administrator invites an Account." | - | H |
| SSS-FG-ORG-R5V | OA | Mycelium Forge shall provide operations for an Organization Administrator to assign and revoke the Organization Administrator and Organization Member roles and to remove a member, shall grant the Organization Administrator role only upon the recipient's explicit acceptance, and shall refuse any operation that would leave the Organization without an Organization Administrator, when "an Organization Administrator manages membership." | - | H |
| SSS-FG-ORG-M6W | OA, OM | Mycelium Forge shall display to a member of an Organization the Accounts belonging to that Organization together with the organization role each holds, when "a member views the Organization." | - | M |
| SSS-FG-ORG-P7X | OA, OM | Mycelium Forge shall remove an Account from an Organization on that Account's own request, and shall refuse the removal where it would leave the Organization without an Organization Administrator or leave a package without an individual-Account Owner, when "a member leaves an Organization." | - | M |
| SSS-FG-ORG-B8Y | OA | Mycelium Forge shall provide operations for an Organization Administrator to configure whether Organization Members may create new packages in the Organization's scope, and to configure the default visibility applied to new packages in that scope, when "an Organization Administrator configures the Organization." | - | M |
| SSS-FG-ORG-A9Z | IA | Mycelium Forge shall display to the Installation Administrator a list of all Organizations showing name, slug, creation date, member count, package count and status, and shall provide operations to create, update, suspend, reactivate and delete any Organization and to add and remove Accounts to and from any Organization with a specified role, when "the Installation Administrator manages Organizations." | - | H |
...
| SSS-CC-EXT-NO1 | - | Mycelium Fabric shall deliver notifications through HTTP webhook channels, with webhook payloads signed using a shared secret and delivered over TLS 1.2 or later, when "a notification channel has been configured for an organisation." | - | M |
| SSS-CC-EXT-OB1 | - | Mycelium Fabric and Mycelium Forge shall expose a Prometheus-compatible `/metrics` endpoint and an OpenTelemetry OTLP exporter for traces and metrics, authenticated through the installation's observability credentials, when "a metrics or tracing collector polls or receives from the endpoint." | - | M |
...
| SSS-FB-OBS-D2B | - | Mycelium Fabric and Mycelium Forge shall emit OpenTelemetry-compatible distributed traces covering inbound HTTP requests, outbound calls between Bloom, Fabric, Forge, and the database, background jobs, and SignalR notification flows, when "a request or background task executes." | - | M |
| SSS-FB-OBS-M3C | - | Mycelium Fabric and Mycelium Forge shall expose a Prometheus-compatible `/metrics` endpoint publishing counters, gauges, and histograms for request rates, request latency, error rates, commit throughput, active SignalR connections, queue depths, and resource utilisation, when "a metrics scraper polls the endpoint." | - | M |
