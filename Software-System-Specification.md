...
| SSS-FB-VAR-V1 | - | Mycelium Fabric shall reject a commit that sets `isVariation = true` on an Enumeration Definition, returning a validation error identifying the offending element, when "a client submits such a commit." | SysML 7.6 | M |  |
| SSS-FB-VAR-V2 | - | Mycelium Fabric shall reject a commit in which a variant membership is owned by an element that is not a variation, returning a validation error identifying the offending element, when "a client submits such a commit." | SysML 7.6 | M |  |
| SSS-FB-VAR-V3 | - | Mycelium Fabric shall reject a commit in which a variant is not a valid usage of the variation's type, returning a validation error identifying the non-conforming variant, when "a client submits such a commit." | SysML 7.6 | M |  |
