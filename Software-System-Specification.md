...
| ID | Roles | Requirement | Ref | Prio |
|----|-------|-------------|-----|------|
| SSS-FG-AUTH-S1A | IA, OA, PA, PT, VW | Mycelium Forge shall authenticate users over OIDC against the identity provider it ships with, and shall federate to an external identity provider where one is configured, so that a Forge installation operates without requiring Mycelium Fabric to be present, when "a user signs in to Mycelium Forge through the web interface, the HTTP API, or the Forge client library." | - | H |
